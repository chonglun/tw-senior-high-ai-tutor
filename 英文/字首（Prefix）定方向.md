...
graph LR
    subgraph 空間方位字首大腦地圖
        Up["<b>super- / sur-</b><br>(在...上方 / 超過)"] 
        In["<b>in- / im-</b><br>(向內 / 進入)"]
        Center((<b>CORE</b><br>核心字根))
        Out["<b>ex- / e-</b><br>(向外 / 離開)"]
        Down["<b>sub- / de-</b><br>(在...下方 / 向下)"]
	    ...
	    In --> ex2["<b>im</b>port (進口)<br><b>in</b>spect (內視/檢查)"]
    end

    %% 實例連結
    Up --> ex1["<b>super</b>man (超人)<br><b>sur</b>plus (過剩)"]
    Out --> ex3["<b>ex</b>port (出口)<br><b>e</b>ject (彈出)"]
    Down --> ex4["<b>sub</b>way (地鐵)<br><b>de</b>cline (下降/婉拒)"]

    style Center fill:#ff9999,stroke:#333
    style Up fill:#e8f5e9,stroke:#2e7d32
    style Down fill:#efebe9,stroke:#4e342e
...
graph LR
    %% 節點定義
    Past["<b>pre- / fore-</b><br>(在...之前 / 預先)"] --> Present((<b>現在 / 動作當下</b>))
    Present --> Future["<b>post-</b><br>(在...之後 / 延後)"]
    Present -.-> |"<b>re-</b> (重新/往回)"| Past

    %% 實例標註
    classDef box fill:#f5f5f5,stroke:#9e9e9e;
    class Past,Future box;

    %% 加上生動標籤
    click Past "- <b>pre</b>dict (預測)<br>- <b>fore</b>head (前額)"
    click Future "- <b>post</b>pone (延期)<br>- <b>post</b>war (戰後)"
    click Present "- <b>re</b>play (重播)<br>- <b>re</b>turn (走回頭路/歸還)"
```
...
```mermaid
graph LR
    Word["<b>【核心字根 / 單字】</b>"] --> Positive{正向/原生}
    Word --> Negative{負向/否定}

    Positive --> |"<b>pro-</b> (支持/往前)"| p1["<b>pro</b>-war (支持戰爭的)"]
    Positive --> |"<b>co- / con-</b> (共同/凝聚)"| p2["<b>co</b>work (合作)"]

    Negative --> |"<b>un- / in- / il-</b> (純粹否定)"| n1["<b>un</b>happy (不快樂)<br><b>il</b>legal (不合法)"]
    Negative --> |"<b>dis-</b> (分開/消失)"| n2["<b>dis</b>appear (消失)<br><b>dis</b>agree (不同意)"]
    Negative --> |"<b>anti- / contra-</b> (反對/對抗)"| n3["<b>anti</b>virus (抗病毒)<br><b>contra</b>dict (矛盾)"]

    style Word fill:#eceff1,stroke:#37474f
    style Positive fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    style Negative fill:#ffebee,stroke:#c62828,stroke-width:2px
```
...
```mermaid
graph LR
    Start["<b>原生字首：in-</b><br>(核心義：不 / 向內)"] --> Judge{"檢查字根開頭字母？"}

    Judge --> |"遇到 <b>b / m / p</b>"| M["<b>變身為 im-</b><br>(雙唇音同化)"]
    Judge --> |"遇到 <b>l</b>"| L["<b>變身為 il-</b><br>(連音同化)"]
    Judge --> |"遇到 <b>r</b>"| R["<b>變身為 ir-</b><br>(連音同化)"]
    Judge --> |其他多數字母| Normal["<b>維持原樣 in-</b>"]

    %% 實例
    M --> exM["in + port = <b>import</b> (進口)"]
    L --> exL["in + legal = <b>illegal</b> (不合法)"]
    R --> exR["in + regular = <b>irregular</b> (不規則)"]
    Normal --> exN["in + correct = <b>incorrect</b> (不正確)"]
...
- [字根（Root）定核心](字根（Root）定核心.md)
- [字尾（Suffix）定詞性](字尾（Suffix）定詞性.md)
- 字尾（Suffixes）詞性判斷
- [核心字彙與字根字首](核心字彙與字根字首.md)
- 閱讀理解策略
- 篇章結構解題法
- 同源字與一字多義陷阱
